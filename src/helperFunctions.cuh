...
__device__ void findNextVertex(CSRGraph graph,int* visited_s, unsigned int* startingVertex, unsigned int* backtrackingIndices_s,unsigned int* depth, 
    ...
    *startingVertex2 = *startingVertex;
    *depth2 = *depth;
        ...
        *foundNeighbor = false;
        for(; backtrackingIndices_s[*depth] < end-start; backtrackingIndices_s[*depth]++) { // Delete Neighbors of startingVertex
            if (graph.matching[*startingVertex]>-1 && !visited_s[graph.matching[*startingVertex]]){
                ...
                printf("matched edge %d->%d visited %d\n", *startingVertex, neighbor, visited_s[neighbor]);
            } else {
                neighbor = graph.dst[start + backtrackingIndices_s[*depth]];
                printf("unmatched edge %d->%d visited %d\n", *startingVertex, neighbor, visited_s[neighbor]);
            }
            if (!visited_s[neighbor])
            {
                printf("matched edge %d->%d visited %d breaking\n", *startingVertex, neighbor, visited_s[neighbor]);
                *foundNeighbor = true;
                *startingVertex2 = neighbor;
                *depth2 += 1;
                break;
            }
        }
...
// Set foundNeighbor, this variable will determine whether I pop off the stack or push and continue.
__device__ void prepare_right_child_DFS(unsigned int* startingVertex, unsigned int* backtrackingIndices_s,unsigned int* depth){
    if (threadIdx.x==0){
        backtrackingIndices_s[*depth]+=1;
    }
    __syncthreads();
}
...
// Set foundNeighbor, this variable will determine whether I pop off the stack or push and continue.
__device__ void prepare_left_child_DFS(int* visited_s, unsigned int* backtrackingIndices_s,unsigned int* startingVertex, unsigned int* depth, unsigned int* startingVertex2, unsigned int* depth2){
    if (threadIdx.x==0){
        backtrackingIndices_s[*depth]-=1;
        visited_s[*startingVertex2]=1;
        *depth=*depth2;
        *startingVertex = *startingVertex2;
    }
    __syncthreads();
}
