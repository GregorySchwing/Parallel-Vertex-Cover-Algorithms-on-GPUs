...
void add_edges_to_unmatched_from_last_vertex_gpu_csc(CSRGraph & graph,CSRGraph & graph_d,struct match * m,int exec_protocol){
 ...
 IndexIterator indices_end = thrust::copy_if(thrust::make_counting_iterator(0),
  ...
  cudaMemcpy(graph.matching, m_d, graph.vertexNum*sizeof(*graph.unmatched_vertices), cudaMemcpyDeviceToHost);
  cudaMemcpy(graph_d.matching, m_d, graph.vertexNum*sizeof(*graph.unmatched_vertices), cudaMemcpyDeviceToHost);
