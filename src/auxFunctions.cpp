...
void setBlockDimAndUseGlobalMemoryEdmonds(Config &config, CSRGraph graph, int maxSharedMemPerSM, long long maxGlobalMemory, int maxNumThreadsPerSM,
								   int maxThreadsPerBlock, int maxThreadsPerMultiProcessor, int numOfMultiProcessors, int minimum)
{
	...
	if (config.blockDim)
	{
		...
		minStackSize = ((long long)(graph.vertexNum + 1) * (long long)sizeof(int)) * NumBlocks;
	}
	else
	{
		minStackSize = ((long long)(graph.vertexNum + 1) * (long long)sizeof(int)) * minNumBlocks;
	}
	...
	{
		globalListSize = (long long)(graph.vertexNum + 1) * (long long)sizeof(int);
	}
	else
	{
		globalListSize = 0;
	...
	for (long long i = 32; i < 1024; i = i * 2)
	{
