...
#else
__global__ void GlobalWorkList_shared_DFS_bits_kernel(Stacks stacks, unsigned int * minimum, unsigned int * solution_mutex, WorkList workList, CSRGraph graph, Counters* counters, 
    ...
    int stackTop = -1;
    unsigned int stackSize = (stacks.minimum + 2);
    volatile uint32_t * stackVertexDegrees = &stacks.stacks_bits[blockIdx.x * stackSize * (graph.vertexNum+31)/32];
    ...
    while(true){
        if(dequeueOrPopNextItr) {
            if(stackTop != -1) { // Local stack is not empty, pop from the local stack
                ...
                popStack_DFS_bits(graph.vertexNum, vertexDegrees_s, &numDeletedVertices, &edgeIndex, stackVertexDegrees, stackNumDeletedVertices, &stackTop);               
                ...
                if(!dequeue_DFS_bits(vertexDegrees_s, workList, graph.vertexNum, &numDeletedVertices, &edgeIndex)) {   
...
//            minimum_s = atomicOr(minimum,graph.matching[numDeletedVertices]==-1&&!vertexDegrees_s[numDeletedVertices]);
        ...
        if(minimum_s||edgeIndex>=(graph.srcPtr[numDeletedVertices+1]-graph.srcPtr[numDeletedVertices])) { // Reached the bottom of the tree, no minimum vertex cover found
            ...
            findUnmatchedNeighbor_bits(graph,numDeletedVertices,&edgeIndex,&neighbor,&foundNeighbor,vertexDegrees_s);
            ...
            if(!foundNeighbor) { // Reached the bottom of the tree, minimum vertex cover possibly found
                ...
                */
                ...
                if (foundNeighbor==2){
                    ...
                    //deleteNeighborsOfMaxDegreeVertex(graph,vertexDegrees_s, &numDeletedVertices, vertexDegrees_s2, &numDeletedVertices2, maxDegree, maxVertex);
                    prepareRightChild_bits(graph,vertexDegrees_s,&numDeletedVertices,&edgeIndex,vertexDegrees_s2,&numDeletedVertices2,&edgeIndex2);
                    ...
                    if(checkThreshold(workList)){
                        ...
                        enqueueSuccess = enqueue_DFS_bits(vertexDegrees_s2, workList, graph.vertexNum, &numDeletedVertices2,&edgeIndex2);
                    } else  {
                    ...
                    if(!enqueueSuccess) {
                        ...
                        pushStack_DFS_bits(graph.vertexNum, vertexDegrees_s2, &numDeletedVertices2, &edgeIndex2,stackVertexDegrees, stackNumDeletedVertices, &stackTop);                    
                ...
                //deleteMaxDegreeVertex(graph, vertexDegrees_s, &numDeletedVertices, maxVertex);
                prepareLeftChild_bits(vertexDegrees_s,&numDeletedVertices,&edgeIndex,&neighbor);
