...
CSRGraph createCSRGraphFromFile(const char *filename)
{
	...
	for (unsigned int i = 0; i < edgeNum; i++)
	{
		unsigned int v0, v1;
		fscanf(fp, "%u%u", &v0, &v1);
		// Shift all vertex indices down by 1
		edgeList[0][i] = v0-1;
		edgeList[1][i] = v1-1;
	}
...
void setBlockDimAndUseGlobalMemoryDFS_NoStack(Config &config, CSRGraph graph, int maxSharedMemPerSM, long long maxGlobalMemory, int maxNumThreadsPerSM,
								   int maxThreadsPerBlock, int maxThreadsPerMultiProcessor, int numOfMultiProcessors, int minimum)
{
	...
	*/
	...
	if (config.version == HYBRID)
	{
		...
		globalListSize = (long long)2*graph.edgeNum * (long long)sizeof(uint64_t);
	}
	else
	{
		globalListSize = 0;
	...
	long long integerArrays = 6 * ((long long)(graph.vertexNum) * (long long)sizeof(int));
	long long pairArrays = ((long long)(graph.vertexNum) * (long long)sizeof(uint64_t));
	...
	long long maxNumBlocksGlobalMem = MIN(availableGlobalMem / (memoryRequiredPerBlock), maxNumThreadsPerSM * numOfMultiProcessors / 64);
	printf("maxNumBlocksGlobalMem %lld\n",maxNumBlocksGlobalMem);
	long long minNumBlocksGlobalMem = MIN(availableGlobalMem / (memoryRequiredPerBlock), maxNumThreadsPerSM * numOfMultiProcessors / maxThreadsPerBlock);
	printf("minNumBlocksGlobalMem %lld\n",minNumBlocksGlobalMem);
	long long minBlockDimGlobalMem = maxNumThreadsPerSM * numOfMultiProcessors / maxNumBlocksGlobalMem;
	printf("minBlockDimGlobalMem %lld\n",minBlockDimGlobalMem);
	...
	for (long long blockDim = minBlockDim; blockDim <= maxBlockDim; blockDim *= 2)
	{
		...
		long long sharedMemNeeded = memoryRequiredPerBlock + (numSharedMemVariables * sizeof(int));
